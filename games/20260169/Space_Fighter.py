...
BASE_PLAYER_MAX_HP = 20
BASE_PLAYER_DAMAGE = 99
REROLL_COST = 10
...
CURRENT_BGM_BASE_VOLUME = 0.35
BGM_VOLUME = 0.38
FX_VOLUME = 1.0


def play_bgm(track_path, volume=0.35):
    global CURRENT_BGM_PATH, CURRENT_BGM_BASE_VOLUME
    try:
        if CURRENT_BGM_PATH == track_path:
            CURRENT_BGM_BASE_VOLUME = volume
...
def play_sound(sound, volume=1.0):
    if sound:
        sound.set_volume(volume * FX_VOLUME)
        sound.play()


def play_looping_sound(channel, sound, volume=1.0):
    if channel and sound:
        channel.set_volume(volume * FX_VOLUME)
...
PLAYER_SHIELD_SOUND = load_sound_from_base64_file("Assets", "sound", "459782__metzik__deflector-shield.wav")
PLAYER_BEAM_SOUND = load_sound_from_base64_file("Assets", "sound", "733999__velcronator__laser-06.wav")
PLAYER_BEAM_CHANNEL = pygame.mixer.Channel(1)
...
class Player:
    def __init__(self):
        ...
        self.sprite_scale = 2
        self.parry_radius = 60
        self.parry_duration = 0.22
    ...
    def reset_for_new_game(self):
        ...
        self.beam_timer = 0.0
        self.beam_sound_playing = False
        stop_channel(PLAYER_BEAM_CHANNEL)
    ...
    def update(self, dt, keys):
        ...
        if self.beam_timer > 0:
            self.beam_timer -= dt
            if not self.beam_sound_playing:
                play_looping_sound(PLAYER_BEAM_CHANNEL, PLAYER_BEAM_SOUND, 0.35)
                self.beam_sound_playing = True
        elif self.beam_sound_playing:
            stop_channel(PLAYER_BEAM_CHANNEL)
            self.beam_sound_playing = False
    ...
    def try_shoot(self, now, bullets):
        if self.beam_ready:
            ...
            play_sound(PLAYER_SHOOT_SOUND, 0.35)
            return
        if self.beam_timer > 0:
            return
    ...
    def try_start_parry(self):
        if self.beam_timer > 0:
            return
        if self.parry_timer > 0 or self.parry_cooldown_timer > 0:
            return
        self.parry_timer = self.parry_duration
        self.parry_cooldown_timer = self.parry_cooldown
        play_sound(PLAYER_SHIELD_SOUND, 0.8)
    ...
    def register_parry_success(self):
        self.parry_count += 1
        if self.parry_count >= 3:
            self.parry_count = 0
            self.beam_ready = True

    def has_active_beam(self):
        return self.beam_timer > 0
    ...
    def draw(self, surface):
        ...
        if self.is_parrying():
            alpha = 110 + int(80 * abs(math.sin(self.parry_timer * 20)))
            ring_surface = pygame.Surface((self.parry_radius * 2 + 16, self.parry_radius * 2 + 16), pygame.SRCALPHA)
            center = (self.parry_radius + 8, self.parry_radius + 8)
            pygame.draw.circle(ring_surface, (80, 240, 255, alpha), center, self.parry_radius, 4)
            surface.blit(ring_surface, (self.x - center[0], self.y - center[1]))

        if self.has_active_beam():
            beam_surface = pygame.Surface((self.beam_width * 3, int(self.y)), pygame.SRCALPHA)
            beam_x = self.beam_width * 1.5
            pygame.draw.line(beam_surface, (80, 240, 255, 170), (beam_x, int(self.y)), (beam_x, 0), self.beam_width)
            pygame.draw.line(beam_surface, (255, 255, 255, 210), (beam_x, int(self.y)), (beam_x, 0), max(6, self.beam_width // 3))
            surface.blit(beam_surface, (self.x - beam_x, 0))
...
class Boss:
    def __init__(self, stage):
        ...
        stage_mul = 1.5 if stage == 1 else (2.25 if stage == 2 else 4.5)
        boss_hp = 500
        ...
        if self.variant == "bullet_focus":
            if self.attack_timer >= 1.35:
                ...
                if roll < 0.24:
                    ...
                    self.fire_double_fan_to_player(player, 4, 235, 1)
                elif roll < 0.84:
                    self.fire_spiral_bullets(14, 190, 1)
                else:
                    self.fire_targeted_burst(player, 9, 260, 1)
            ...
            if self.pattern_timer >= 5.8:
                ...
                if roll < 0.25:
                    ...
                    self.spawn_cross_laser_random()
                elif roll < 0.86:
                    self.spawn_double_diagonal_lasers()
                else:
                    self.spawn_diagonal_laser_random()
        else:
            if self.attack_timer >= (1.0 / 1.5):
                self.attack_timer = 0.0
                roll = random.random()
                if roll < 0.24:
                    self.spawn_vertical_laser_near_player(player)
                elif roll < 0.48:
                    self.spawn_horizontal_laser_near_player(player)
                elif roll < 0.68:
                    self.spawn_cross_laser_near_player(player)
                elif roll < 0.84:
                    self.spawn_double_vertical_laser_near_player(player)
                else:
                    self.fire_fan_to_player(player, 5, 220, 1)

            if self.pattern_timer >= (5.0 / 1.5):
                self.pattern_timer = 0.0
                roll = random.random()
                if roll < 0.20:
                    self.spawn_pulse_circle_laser()
                elif roll < 0.38:
                    self.spawn_vertical_laser_near_player(player, width=72, warning=0.85, active=0.6)
                elif roll < 0.56:
                    self.spawn_horizontal_laser_near_player(player, width=72, warning=0.85, active=0.6)
                elif roll < 0.74:
                    self.spawn_cross_laser_near_player(player, width=64, warning=0.95, active=0.6)
                elif roll < 0.88:
                    self.spawn_diagonal_laser_near_player(player, width=20, warning=0.9, active=0.6)
                else:
                    self.spawn_laser_box_around_player(player, width=54, warning=0.9, active=0.55)
    ...
    def fire_double_fan_to_player(self, player, count, speed, damage):
        dx = player.x - self.x
        dy = player.y - self.y
        base = math.atan2(dy, dx)
        for offset in (-0.42, 0.42):
            start = base + offset - 0.20
            end = base + offset + 0.20
            for i in range(count):
                ang = start if count == 1 else start + (end - start) * i / (count - 1)
                self.enemy_bullets.append(
                    EnemyBullet(self.x, self.y, math.cos(ang), math.sin(ang), speed, damage, radius=8, color=PURPLE)
                )

    def fire_spiral_bullets(self, count, speed, damage):
        base = self.move_t * 3.2
        for i in range(count):
            ang = base + i * 0.38
            self.enemy_bullets.append(
                EnemyBullet(self.x, self.y, math.cos(ang), math.sin(ang), speed, damage, radius=8, color=ORANGE)
            )

    def fire_targeted_burst(self, player, count, speed, damage):
        dx = player.x - self.x
        dy = player.y - self.y
        base = math.atan2(dy, dx)
        for i in range(count):
            ang = base + random.uniform(-0.55, 0.55)
            shot_speed = speed + i * 6
            self.enemy_bullets.append(
                EnemyBullet(self.x, self.y, math.cos(ang), math.sin(ang), shot_speed, damage, radius=7, color=PURPLE)
            )
    ...
    def spawn_vertical_laser_near_player(self, player, width=64, warning=1.0, active=0.55):
        lx = int(clamp(player.x + random.uniform(-110, 110), 90, WIDTH - 90))
        self.lasers.append(LaserAttack(lx, width, warning, active, 1))

    def spawn_horizontal_laser_near_player(self, player, width=64, warning=1.0, active=0.55):
        ly = int(clamp(player.y + random.uniform(-90, 90), 90, HEIGHT - 90))
        self.lasers.append(LaserAttack(ly, width, warning, active, 1, axis="horizontal"))

    def spawn_cross_laser_near_player(self, player, width=64, warning=1.0, active=0.55):
        self.spawn_vertical_laser_near_player(player, width=width, warning=warning, active=active)
        self.spawn_horizontal_laser_near_player(player, width=width, warning=warning, active=active)
    ...
    def spawn_double_diagonal_lasers(self, width=18, warning=0.9, active=0.65):
        self.line_lasers.append(PulseLineLaser(0, HEIGHT, WIDTH, 0, warning, active, width, 1))
        self.line_lasers.append(PulseLineLaser(0, 0, WIDTH, HEIGHT, warning, active, width, 1))

    def spawn_diagonal_laser_near_player(self, player, width=18, warning=0.9, active=0.65):
        offset = random.uniform(-120, 120)
        if random.random() < 0.5:
            x1, y1 = 0, clamp(player.y + offset, 0, HEIGHT)
            x2, y2 = WIDTH, clamp(player.y + offset - HEIGHT * 0.5, 0, HEIGHT)
        else:
            x1, y1 = 0, clamp(player.y + offset - HEIGHT * 0.5, 0, HEIGHT)
            x2, y2 = WIDTH, clamp(player.y + offset, 0, HEIGHT)
        self.line_lasers.append(PulseLineLaser(x1, y1, x2, y2, warning, active, width, 1))

    def spawn_double_vertical_laser_near_player(self, player, width=56, warning=0.9, active=0.55):
        offset = random.uniform(60, 120)
        left_x = int(clamp(player.x - offset, 90, WIDTH - 90))
        right_x = int(clamp(player.x + offset, 90, WIDTH - 90))
        self.lasers.append(LaserAttack(left_x, width, warning, active, 1))
        self.lasers.append(LaserAttack(right_x, width, warning, active, 1))

    def spawn_laser_box_around_player(self, player, width=54, warning=0.9, active=0.55):
        x_offset = random.uniform(70, 120)
        y_offset = random.uniform(60, 100)
        self.lasers.append(LaserAttack(int(clamp(player.x - x_offset, 90, WIDTH - 90)), width, warning, active, 1))
        self.lasers.append(LaserAttack(int(clamp(player.x + x_offset, 90, WIDTH - 90)), width, warning, active, 1))
        self.lasers.append(LaserAttack(int(clamp(player.y - y_offset, 90, HEIGHT - 90)), width, warning, active, 1, axis="horizontal"))
        self.lasers.append(LaserAttack(int(clamp(player.y + y_offset, 90, HEIGHT - 90)), width, warning, active, 1, axis="horizontal"))
...
class Upgrade:
    ...
    def apply(self, player):
        ...
        elif self.key == "parry_cooldown":
            player.parry_cooldown = max(0.25, player.parry_cooldown * self.value)
        elif self.key == "beam_duration":
            player.beam_duration += self.value


def make_upgrade_pool():
    return [
        Upgrade("발사 탄수 증가", "한 번에 발사하는 탄수가 1 증가", 140, "multishot", 1),
        ...
        Upgrade("패링 쿨타임 감소", "패링 쿨타임 25% 감소", 60, "parry_cooldown", 0.75),
        Upgrade("레이저 지속시간 증가", "레이저 지속시간 +2초", 65, "beam_duration", 2.0),
    ]
...
class Game:
    def __init__(self):
        ...
        self.pause_return_state = STATE_FARMING
        self.settings_return_state = STATE_PAUSED
        self.pause_button_rects = {}
        self.settings_slider_rects = {}

    def set_bgm_volume(self, value):
        global BGM_VOLUME
        BGM_VOLUME = clamp(value, 0.0, 1.0)
        try:
            pygame.mixer.music.set_volume(CURRENT_BGM_BASE_VOLUME * BGM_VOLUME)
        except:
            pass

    def set_fx_volume(self, value):
        global FX_VOLUME
        FX_VOLUME = clamp(value, 0.0, 1.0)
        if self.player.beam_sound_playing:
            PLAYER_BEAM_CHANNEL.set_volume(0.35 * FX_VOLUME)
    ...
    def open_pause_menu(self):
        if self.state in (STATE_FARMING, STATE_BOSS, STATE_BOSS_WARNING, STATE_SHOP):
            self.pause_return_state = self.state
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            if self.state in (STATE_FARMING, STATE_BOSS, STATE_BOSS_WARNING):
        ...
        if self.state == STATE_START:
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                mx, my = event.pos
                if self.pause_button_rects.get("continue") and self.pause_button_rects["continue"].collidepoint(mx, my):
                    self.resume_from_pause()
                elif self.pause_button_rects.get("settings") and self.pause_button_rects["settings"].collidepoint(mx, my):
                    self.open_settings_menu()
                elif self.pause_button_rects.get("quit") and self.pause_button_rects["quit"].collidepoint(mx, my):
                    pygame.quit()
                    sys.exit()
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                mx, my = event.pos
                if self.settings_slider_rects.get("back") and self.settings_slider_rects["back"].collidepoint(mx, my):
                    self.state = self.settings_return_state
                for key in ("fx", "bgm"):
                    if key in self.settings_slider_rects:
                        bar_rect = self.settings_slider_rects[key]
                        if bar_rect.collidepoint(mx, my):
                            value = (mx - bar_rect.left) / max(1, bar_rect.width)
                            if key == "fx":
                                self.set_fx_volume(value)
                            else:
    ...
    def update(self, dt):
        ...
        if self.state in (STATE_PAUSED, STATE_SETTINGS):
            return
        ...
        if self.state == STATE_FARMING:
            ...
            self.handle_parry_collisions()
            self.handle_bullet_enemy_collisions()
            self.handle_parried_bullet_collisions()
            self.handle_player_beam_collisions(dt)
    ...
    def parry_bullet(self, bullet):
        if not bullet.alive or bullet.parried:
            return
        bullet.vx = 0
        bullet.vy = -1
        bullet.speed = max(540, bullet.speed * 1.3)
        bullet.damage = max(1, self.player.damage)
        bullet.color = CYAN
        bullet.parried = True
        self.player.register_parry_success()
        self.add_hit_effect(bullet.x, bullet.y, CYAN, count=8, big=False)

    def handle_parry_collisions(self):
        if not self.player.is_parrying():
            return
        bullet_groups = [self.enemy_bullets]
        for enemy in self.enemies:
            bullet_groups.append(enemy.enemy_bullets)
        if self.boss:
            bullet_groups.append(self.boss.enemy_bullets)
        for bullets in bullet_groups:
            for bullet in bullets:
                ...
                if circle_collision(
                    ...
                    self.player.x,
                    self.player.y,
                    self.player.parry_radius,
                ):
                    self.parry_bullet(bullet)

    def handle_parried_bullet_collisions(self):
        bullet_groups = [self.enemy_bullets]
        for enemy in self.enemies:
            bullet_groups.append(enemy.enemy_bullets)
        if self.boss:
        ...
        for bullets in bullet_groups:
            for bullet in bullets:
                if not bullet.alive or not bullet.parried:
                    continue
                if self.boss and self.boss.alive and self.boss.body_hit_test(bullet):
                    self.boss.take_damage(bullet.damage)
                    self.add_hit_effect(bullet.x, bullet.y, CYAN, count=10, big=False)
                    bullet.alive = False
                    continue
                for enemy in self.enemies:
                    if not enemy.alive:
                    ...
                    if circle_collision(bullet.x, bullet.y, bullet.radius, enemy.x, enemy.y, enemy.radius):
                        ...
                        self.add_hit_effect(bullet.x, bullet.y, CYAN, count=8, big=enemy.big)
                        bullet.alive = False
                        if enemy.hp <= 0:
    ...
    def handle_player_beam_collisions(self, dt):
        if not self.player.has_active_beam():
            return
        beam_damage = max(1.0, self.player.damage * self.player.bullet_count * 5) * dt
        for enemy in self.enemies:
            if not enemy.alive:
                continue
            if line_circle_collision(
                ...
                enemy.y,
                enemy.radius,
                self.player.beam_width,
            ):
                enemy.hp -= beam_damage
                enemy.hit_flash_timer = max(enemy.hit_flash_timer, 0.04)
                if enemy.hp <= 0:
                    if enemy.enemy_bullets:
                        self.enemy_bullets.extend(enemy.enemy_bullets)
                        enemy.enemy_bullets.clear()
        ...
        if self.boss and self.boss.alive:
            hit_boss = False
            if self.boss.stage == 3:
                body_rect = pygame.Rect(0, 0, 180, 140)
                body_rect.center = (self.boss.x, self.boss.y)
                hit_boss = rect_circle_collision(
                    body_rect,
                    self.player.x,
                    self.boss.y,
                    max(self.player.beam_width // 2, 8),
                ) and self.boss.y < self.player.y
            else:
                hit_boss = line_circle_collision(
                    self.player.x,
                    self.player.y,
                    self.player.x,
                    -40,
                    self.boss.x,
                    self.boss.y,
                    self.boss.body_radius,
                    self.player.beam_width,
                )
            if hit_boss:
                self.boss.take_damage(beam_damage)
    ...
    def handle_bullet_enemy_collisions(self):
        for bullet in self.player_bullets:
            ...
            for enemy in self.enemies:
                if not enemy.alive:
                    continue
                enemy_hit_key = ("enemy", id(enemy))
                if enemy_hit_key in bullet.hit_targets:
                    continue
                if circle_collision(bullet.x, bullet.y, bullet.radius, enemy.x, enemy.y, enemy.radius):
                    bullet.hit_targets.add(enemy_hit_key)
                    enemy.hp -= bullet.damage
                    enemy.hit_flash_timer = 0.08
    ...
    def handle_boss_bullet_collisions(self):
        ...
        for bullet in self.player_bullets:
            if not bullet.alive:
                continue
            boss_hit_key = ("boss", id(self.boss))
            if boss_hit_key in bullet.hit_targets:
                continue
            if self.boss.body_hit_test(bullet):
                bullet.hit_targets.add(boss_hit_key)
                self.boss.take_damage(bullet.damage)
                self.add_hit_effect(bullet.x, bullet.y, CYAN, count=8, big=False)
    ...
    def draw(self, surface):
        ...
        if self.state == STATE_START:
            ...
            self.draw_state_content(surface, self.pause_return_state)
            self.draw_pause_overlay(surface)
        elif self.state == STATE_SETTINGS:
            base_state = self.pause_return_state if self.settings_return_state == STATE_PAUSED else self.settings_return_state
            self.draw_state_content(surface, base_state)
            self.draw_settings_overlay(surface)
    ...
    def draw_state_content(self, surface, state):
        if state == STATE_FARMING:
            self.draw_farming(surface)
        elif state == STATE_BOSS_WARNING:
            self.draw_boss_warning(surface)
        elif state == STATE_BOSS:
            self.draw_boss(surface)
        elif state == STATE_SHOP:
            self.draw_shop(surface)

    def draw_common_hud(self, surface):
        if self.player.parry_cooldown_timer > 0:
            draw_text(surface, f"패링 쿨타임 {self.player.parry_cooldown_timer:.1f}초", FONT_22, ORANGE, 18, 220)
        else:
            draw_text(surface, "패링 준비 완료", FONT_22, GREEN, 18, 220)
        draw_text(surface, f"스테이지: {self.stage}", FONT_28, WHITE, 18, 12)
        draw_text(surface, f"골드: {self.player.gold}", FONT_28, YELLOW, 18, 44)
        draw_text(surface, f"패링 {self.player.parry_count}/3", FONT_22, CYAN, 18, 76)

        draw_text(surface, f"공격력 {int(self.player.damage)}", FONT_22, LIGHT_GRAY, WIDTH - 222, 18)
        draw_text(surface, f"탄수 {self.player.bullet_count}", FONT_22, LIGHT_GRAY, WIDTH - 222, 46)
        draw_text(surface, f"관통 {self.player.pierce}", FONT_22, LIGHT_GRAY, WIDTH - 222, 74)
        if self.player.beam_ready:
            draw_text(surface, "빔 준비 완료", FONT_22, CYAN, WIDTH - 222, 102)

        for i in range(self.player.max_hp):
            color = RED if i < self.player.hp else BLACK
            pygame.draw.circle(surface, color, (WIDTH - 28 - i * 28, 45), 10)
            pygame.draw.circle(surface, WHITE, (WIDTH - 28 - i * 28, 45), 10, 2)
        draw_text(surface, f"공격력 {int(self.player.damage)}", FONT_22, LIGHT_GRAY, 18, 108)
        draw_text(surface, f"탄수 {self.player.bullet_count}", FONT_22, LIGHT_GRAY, 18, 136)
        draw_text(surface, f"관통 {self.player.pierce}", FONT_22, LIGHT_GRAY, 18, 164)
        if self.player.beam_ready:
            draw_text(surface, "빔 준비 완료", FONT_22, CYAN, 18, 192)
        return

        draw_text(surface, f"스테이지: {self.stage}", FONT_28, WHITE, 18, HEIGHT - 102)
        draw_text(surface, f"골드: {self.player.gold}", FONT_28, YELLOW, 18, HEIGHT - 70)
        draw_text(surface, f"패링 {self.player.parry_count}/3", FONT_22, CYAN, 18, HEIGHT - 38)

        draw_text(surface, f"공격력 {int(self.player.damage)}", FONT_22, LIGHT_GRAY, WIDTH - 222, HEIGHT - 110)
        draw_text(surface, f"탄수 {self.player.bullet_count}", FONT_22, LIGHT_GRAY, WIDTH - 222, HEIGHT - 82)
        draw_text(surface, f"관통 {self.player.pierce}", FONT_22, LIGHT_GRAY, WIDTH - 222, HEIGHT - 54)
        if self.player.beam_ready:
            draw_text(surface, "빔 준비 완료", FONT_22, CYAN, WIDTH - 222, HEIGHT - 26)

        for i in range(self.player.max_hp):
            color = RED if i < self.player.hp else DARK_RED
            pygame.draw.circle(surface, color, (WIDTH - 28 - i * 28, HEIGHT - 26), 10)
            pygame.draw.circle(surface, WHITE, (WIDTH - 28 - i * 28, HEIGHT - 26), 10, 2)
        return
        draw_text(surface, f"Stage: {self.stage}", FONT_28, WHITE, 18, 12)
        draw_text(surface, f"Gold: {self.player.gold}", FONT_28, YELLOW, 18, 44)
        draw_text(surface, f"스테이지: {self.stage}", FONT_28, WHITE, 18, 12)
        draw_text(surface, f"골드: {self.player.gold}", FONT_28, YELLOW, 18, 44)
        ...
        pygame.draw.rect(surface, BLACK, (0, 0, 320, 132))
        pygame.draw.rect(surface, BLACK, (WIDTH - 250, 40, 250, 90))
        draw_text(surface, f"스테이지: {self.stage}", FONT_28, WHITE, 18, 12)
        draw_text(surface, f"골드: {self.player.gold}", FONT_28, YELLOW, 18, 44)
        draw_text(surface, f"공격력 {int(self.player.damage)}", FONT_22, LIGHT_GRAY, WIDTH - 220, 50)
        draw_text(surface, f"탄수 {self.player.bullet_count}", FONT_22, LIGHT_GRAY, WIDTH - 220, 76)
        draw_text(surface, f"관통 {self.player.pierce}", FONT_22, LIGHT_GRAY, WIDTH - 220, 102)
        draw_text(surface, f"패링 {self.player.parry_count}/3", FONT_22, CYAN, 18, 78)
        if self.player.beam_ready:
            draw_text(surface, "빔 준비 완료", FONT_22, CYAN, 18, 104)
    ...
    def draw_start(self, surface):
        draw_text(surface, "Space_Fighter", FONT_72, WHITE, WIDTH // 2, 110, center=True)
        guide = [
            "Stage flow: Farming -> Shop -> Boss",
            "Move: Arrow Keys or WASD",
            "Attack: Left Click",
            "Parry: Right Click",
            "Shop: Left Click buy / Enter next stage / R reroll",
            "Stage 3 boss takes full damage on the main body",
        ]
        y = 240
        for line in guide:
            draw_text(surface, line, FONT_28, LIGHT_GRAY, WIDTH // 2, y, center=True)
            y += 40
        panel = pygame.Rect(120, 210, WIDTH - 240, 310)
        ...
        guide = [
            ...
            "공격: 마우스 좌클릭",
            "패링: 마우스 우클릭",
            "상점: 좌클릭 구매 / Enter 다음 단계 / R 리롤",
            "3스테이지 보스는 몸체를 노려야 한다",
        ]
        y = 240
        for line in guide:
            draw_text(surface, line, FONT_28, LIGHT_GRAY, WIDTH // 2, y, center=True)
    ...
    def draw_pause_overlay(self, surface):
        ...
        panel = pygame.Rect(WIDTH // 2 - 170, HEIGHT // 2 - 170, 340, 300)
        pygame.draw.rect(surface, (35, 40, 60), panel, border_radius=18)
        pygame.draw.rect(surface, WHITE, panel, 2, border_radius=18)
        draw_text(surface, "일시정지", FONT_52, WHITE, WIDTH // 2, panel.top + 48, center=True)

        labels = [("continue", "계속하기"), ("settings", "설정"), ("quit", "종료하기")]
        self.pause_button_rects = {}
        for index, (key, label) in enumerate(labels):
            rect = pygame.Rect(panel.left + 40, panel.top + 110 + index * 62, panel.width - 80, 48)
            self.pause_button_rects[key] = rect
            pygame.draw.rect(surface, (55, 65, 95), rect, border_radius=12)
            pygame.draw.rect(surface, WHITE, rect, 2, border_radius=12)
            draw_text(surface, label, FONT_28, WHITE, rect.centerx, rect.centery, center=True)

    def draw_settings_overlay(self, surface):
        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 170))
        surface.blit(overlay, (0, 0))

        panel = pygame.Rect(WIDTH // 2 - 220, HEIGHT // 2 - 180, 440, 320)
        pygame.draw.rect(surface, (35, 40, 60), panel, border_radius=18)
        pygame.draw.rect(surface, WHITE, panel, 2, border_radius=18)
        draw_text(surface, "설정", FONT_52, WHITE, WIDTH // 2, panel.top + 44, center=True)

        fx_bar = pygame.Rect(panel.left + 100, panel.top + 120, 260, 16)
        bgm_bar = pygame.Rect(panel.left + 100, panel.top + 190, 260, 16)
        self.settings_slider_rects = {"fx": fx_bar, "bgm": bgm_bar}

        draw_text(surface, f"FX 볼륨 {int(FX_VOLUME * 100)}%", FONT_28, WHITE, panel.left + 44, fx_bar.top - 34)
        draw_text(surface, f"BGM 볼륨 {int(BGM_VOLUME * 100)}%", FONT_28, WHITE, panel.left + 44, bgm_bar.top - 34)

        for rect, value, color in ((fx_bar, FX_VOLUME, ORANGE), (bgm_bar, BGM_VOLUME, CYAN)):
            pygame.draw.rect(surface, (80, 84, 110), rect, border_radius=8)
            ...
            pygame.draw.circle(surface, WHITE, (knob_x, rect.centery), 10)

        back_rect = pygame.Rect(panel.left + 120, panel.bottom - 74, 200, 48)
        self.settings_slider_rects["back"] = back_rect
        pygame.draw.rect(surface, (55, 65, 95), back_rect, border_radius=12)
        pygame.draw.rect(surface, WHITE, back_rect, 2, border_radius=12)
        draw_text(surface, "돌아가기", FONT_28, WHITE, back_rect.centerx, back_rect.centery, center=True)
